...
- **Two schedules must differ AFTER Cloudflare normalizes them, not just as written.** A stepped
  range with a nonzero start is normalized to its offset-free form: `1-59/2 * * * *` is accepted,
  and the schedules API echoes it back verbatim, but the scheduler fires it on the EVEN minute
  alongside `*/2 * * * *` and reports `event.cron` as `*/2 * * * *`. Since `event.cron` is the whole
  dispatch, the second unit becomes **unreachable** — it never runs, every tick silently takes the
  fall-through branch, and nothing errors anywhere. Keep every expression offset-free (`*/2`,
  `*/3`, `17 9 * * *`), and have the handler `console.warn` an `event.cron` that matches no
  constant, so the next collapse shows up in `wrangler tail` instead of hiding.
...
- **A Worker running older code fails like a *caller* error, so check the deployed build before debugging the caller.** Input the deployed build doesn't understand takes the same "we don't handle this" branch as genuinely bad input — a webhook Worker answered `200 {"ignored":"no alfred frontmatter block"}` to a perfectly valid block for three days simply because the phase it named had merged but never deployed. `GET /` reports the build (`alfred workers ok (build <sha>)`, or `unstamped` when deployed by hand); diff it against `git rev-parse origin/main` **first**. Suspect this whenever behaviour on Cloudflare contradicts the code in front of you.
- **`✘ [ERROR] fetch failed` from `wrangler deploy` is a transport blip, not your code.** It is raised reaching `api.cloudflare.com` *before* wrangler bundles anything, so the diff it appears to indict is a coincidence — and the "check your network connection" warning misleads, since wrangler's own "newer version available" notice in the same output proves the network was up. The tell is the clock: a real deploy bundles for several seconds before printing `Total Upload:`, while this dies in well under one. The workflow retries 3×, so re-run before debugging the diff; a run still red after that has a genuine fault (auth, config, script size).
- **50 subrequests per INVOCATION on the Free plan (1000 on Paid) — and going over throws part-way, it does not degrade.** Every outbound `fetch` counts: each Supabase REST call, each model call, each Gmail read. So any `for … await` over rows is a budget the row count sets, and it fails as `Too many subrequests by single Worker invocation` only once real data arrives — never in tests, which stub `fetch`. Budget each loop explicitly, cap it, and let the remainder ride the next tick (a cron whose backlog drains over several ticks is almost always fine); put the arithmetic in the cap's doc comment so the next edit can see the ceiling it is spending against. Two traps: work that shares ONE invocation shares ONE budget — units on the same cron expression compete, and splitting them onto separate expressions (which must stay distinct once normalized — see *Cron triggers* above) gives each the full 50; and a throw mid-loop leaves the side effects already committed — a poll that stores nothing but moved no cursor re-runs forever, a half-swept tick has already paid for model calls whose verdicts it never wrote.
...
- **The package's tsconfig `lib` is `ES2022`, so ES2023+ built-ins don't type-check even though the runtime has them.** `String#isWellFormed` / `toWellFormed` (ES2024) fail with "Property … does not exist on type 'string'. Do you need to change your target library?" — never widen the lib to fix it; replace lone surrogates with a regex instead — `/[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(?<![\uD800-\uDBFF])[\uDC00-\uDFFF]/g` → `'\uFFFD'` (`storable()` in `src/wiki/page.ts`). The array side (`toSorted`) is in the eslint skill.
- **A Node-side script under `workers/` needs its own `scripts/tsconfig.json`.** The package tsconfig is `include: ["src"]`, so `scripts/*.ts` sits outside `tsc --noEmit` *and* has no project for the type-aware lint (`projectService`'s `allowDefaultProject` matches only root-level `*.ts`, and ESLint then errors on the file). Add `scripts/tsconfig.json` extending the package one with `"types": ["node"]`, `"rootDir": ".."`, `"allowImportingTsExtensions": true`, `"include": ["."]`. Node runs it directly by stripping the types, but its ESM resolver cannot follow the **extensionless** imports Worker source uses — the failure is an `ERR_MODULE_NOT_FOUND` naming a module the script never mentioned. So write the script's own imports with an explicit `.ts` suffix, and to reach anything under `src/` (whose internal imports you can't change) register a resolve hook that retries a failed *relative* specifier as `<specifier>.ts`: a `scripts/ts-resolve-hooks.mjs` exporting `resolve`, a one-line `scripts/ts-resolve.mjs` calling `register()`, and `node --import ./scripts/ts-resolve.mjs scripts/x.ts` as the npm script. A script that needs nothing from `src/` can skip all of it and stay self-contained.
