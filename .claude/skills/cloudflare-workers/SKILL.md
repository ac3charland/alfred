...
- **`ExportedHandler` fetch signature allows omitting unused parameters.** In TypeScript, you may omit trailing unused parameters: `fetch(): Response` is valid when the handler doesn't need `request`, `env`, or `ctx`. This avoids the `no-unused-vars` lint error without `_`-prefix workarounds.
