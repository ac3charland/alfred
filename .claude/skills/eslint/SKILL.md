...
- **A rule's options *replace*, they don't merge, across overlapping `files` globs** — the last matching block that sets the rule wins outright. So splitting `no-restricted-syntax` (or `no-restricted-imports`) selectors for nested scopes into separate blocks silently drops the broader block's selectors on files the narrower block also matches (e.g. a `components/{tasks,…}/**` block and a `components/**` block both set `no-restricted-syntax` → the feature files only get the narrower block's selectors). Fix: **layer broad→narrow and have each narrower block re-list every selector that also applies to its files** (and re-list that selector's `ignores`), so the one winning block carries them all. Prove it: drop a violating snippet into a file in each scope and confirm every expected selector fires. Different rule *keys* (e.g. `no-restricted-syntax` vs `no-restricted-imports`) don't collide; only same-key options replace. See the regression-ratchet blocks in `frontend/eslint.config.mjs`.
...
- **`npm run lint` runs `eslint --fix`, so running a check REWRITES your source.** A scripted edit (`sed`, a python replace) built from text you read before the gate then matches nothing and silently no-ops — e.g. `--fix` normalizes `() => undefined` to `() => {}`, which the next run reports as `no-empty-function`. Re-read a file after any `check`/`lint` run before editing it by pattern.
- **`unicorn/no-useless-undefined` autofixes a trailing `undefined` ARGUMENT away — so never satisfy a required `x: T | undefined` parameter by passing `undefined`.** `check:fast` is `typecheck && lint && …`, so the run you're watching goes green (tsc saw the argument) and `--fix` then deletes it; the failure surfaces on the *next* run, in a file you didn't touch, looking like a phantom revert. Declare the parameter **optional** (`description?: string`) and omit it at the call site — then there is nothing for the rule to strip. The same trap applies to `return undefined` in a `T | undefined` function and to `useState(undefined)`.
...
- **A `role="option"` / `role="menuitem"` row with an `onClick` still needs its own `onKeyDown`.** `jsx-a11y/click-events-have-key-events` errors on the clickable `<li>` even when the combobox drives selection from the input (arrows + Enter) and the row is only ever reached by mouse. Give the row an `onKeyDown` handling `Enter`/`Space` that calls the same select — it costs three lines and makes the option operable for a client that focuses it directly. Both listbox rows in the app carry it (`shell/search-results-popover.tsx`, `tasks/project-suggest-popover.tsx`).
- **`@typescript-eslint/no-unnecessary-condition` fires on the SECOND `rows[0]?.x` in a row.** A guard that throws — `if (rows[0]?.count !== 3) throw …` — narrows `rows[0]` to non-nullish for everything after it, so the next `rows[0]?.name` is an "unnecessary optional chain" even though the first was fine. Bind the row once (`const row = rows[0]; if (row?.count !== 3) throw …; row.name`) rather than repeating the index; the same narrowing makes `a?.x !== v || a.y !== w` the correct spelling inside one condition. (The `database` integration assertions are written this way.)
...
- **Two test files with the same basename but different extensions (`foo.test.ts` + `foo.test.tsx`) orphan one from the project service** — TS resolves the pair to a single module, leaving the other *"not found by the project service"* and erroring lint, even though both are inside `include`. Before adding a test, check whether one already exists (`ls <name>.test.*`); add to it rather than creating a sibling with a different extension. **This is the failure mode `eslint --cache` hides locally:** the pre-existing file is cached as clean and skipped, so your local `check:fast` is green while CI (fresh checkout, cold cache) re-lints it and fails. The same hiding happens when a file's *types* change under it without the file changing — a widened `scripts/tsconfig.json` turned `response.json()` into an error type in a probe script the cache still held as clean. When a lint error reproduces only in CI, suspect the cache — `rm <pkg>/.eslintcache && npm run lint -w <pkg>` to see what CI sees.
- **Concurrent agents editing the same tree race the same cache**: one agent's clean lint run can stamp a file another agent is mid-edit on as unchanged, so a later error in that file stays hidden locally. Before a push that ends a multi-agent session, delete every package's `.eslintcache` (`find . -name .eslintcache -not -path '*/node_modules/*' -delete`) and run `check:fast` once cold — CI has no cache and will catch whatever a stale entry hid.
