...
- **The same trap applies to `languageOptions`**: `{ ...tseslint.configs.disableTypeChecked, languageOptions: { globals: … } }` throws away the spread's `parserOptions` (including `projectService: false`), silently pushing JS/CJS/MJS files back into the type-aware project service. Re-spread it: `languageOptions: { ...tseslint.configs.disableTypeChecked.languageOptions, globals: … }`.
- **`allowDefaultProject` hard-caps at 8 matched files** — a 9th match makes every run fail with `Parsing error: Too many files (>8) have matched the default project`, but **only on a cold `.eslintcache`** (warm runs pass, so the break surfaces in fresh clones / CI, not the dev machine that introduced it). Keep the list to type-checked files only (the `.storybook/*.ts(x)` files); JS-family files belong to the `disableTypeChecked` block, not this list. After touching either, verify with a cold run: delete `.eslintcache`, then lint.
