...
- **The `overflow-hidden` clip also shaves anything that paints past the content's border box** — a `focus-visible` ring (`ring-2` + `ring-offset-1` ≈ 3px), a glow, a box-shadow. Where the child sits flush against the clip edge (a `flex-1` field), that edge of the ring is cut flat. Fix: pad the inner (fading) layer by ≥ the paint extent (`px-1`/`py-1` = 4px clears the 3px ring) so the effect stays inside the clip. The add-subtask field in `task-row.tsx` uses `addSubtaskRevealClass = 'px-1 py-1'` for exactly this (ALF-112).
