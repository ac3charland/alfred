...
**Anchor a Tailwind class assertion on whole-class boundaries.** `not.toHaveClass(/bg-secondary/)` *fails* on a resting nav link, because its class list carries `hover:bg-secondary/50` — the variant embeds the base name. Write `/(^|\s)bg-secondary(\s|$)/` so only the standalone utility matches.

**A region collapsed by `AnimatedHeightCollapse` (`frontend/components/atoms/animated-height-collapse.tsx`) stays MOUNTED, clipped to `grid-template-rows: 0fr` inside an `overflow-hidden` div.** Playwright's visibility check ignores clipping from an ancestor's overflow, so `getByText(...)` + `toBeHidden()` FAILS on the clipped element. The wrapper is marked `aria-hidden`/`inert` when closed instead, so assert through the accessibility tree: `getByRole(...)` skips `aria-hidden` subtrees, giving `row.getByRole('heading', { name })` a clean hidden-when-closed / visible-when-open pair (`frontend/e2e/reader-shell.spec.ts`, the overview journey). RTL's `queryByRole` already respects `aria-hidden` the same way.

A spec that hard-codes the module switcher's link count (`mobile-module-switch.spec.ts`'s `toHaveCount(4)`) is the one place the switcher's arity lives outside `lib/modules.ts` — adding a module means updating it too.

**Produce a regression guard's precondition in the test, not from a neighbouring component — and inject it as a STYLE CHANGE, never as a DOM node React never rendered.** A FLIP-jank spec sourced its "something above the list shifted" precondition from whichever card happened to sit above the list, and broke the day that card moved to another view; the guard is about the list's own measurement, so `page.evaluate` the shift in directly. But a spacer element inserted before a React-rendered list is not a stable part of React's child order — the next render of that parent re-inserts the list around it and the shift silently undoes itself, at a moment the test does not control. In a spec that samples positions frame by frame that undo lands inside the sampling window about half the time and reads as a one-frame jump of the shift's whole height: a ~50% flake that looks exactly like the jank under test. Set a style on an element the component is not styling (`parent.style.paddingTop = '120px'`) and the precondition survives every re-render.
...
- **Run one spec through the npm script, never the `playwright` binary** (CLAUDE.md's back-pressure rule): `npm run test:e2e -w frontend -- <name-filter>`. npm appends everything after `--` to the end of the script, so the filter (and flags like `--reporter=line`) reach `playwright test` intact — while `node scripts/setup-chromium.mjs` still runs first, which a bare `npx playwright test` skips.
- **Only one e2e run can be in flight at a time.** The mock and the Next dev server bind fixed ports (`MOCK_PORT` 54331 in `e2e/support/constants.ts`, port 3000 in `playwright.config.ts`'s `webServer`), so a second run started while one is already up either fails with a port-in-use error or, worse, talks to the *other* run's mock and reports confusing failures. Confirm nothing is running before starting one — but `pgrep -f mock-supabase.mjs` always "finds" a match: the pattern also matches the `pgrep` invocation's own command line. Anchor it: `pgrep -f "^node scripts/mock-supabase.mjs"`.
- **A "lands at the top" assertion (`scrollY` → 0) after an in-app navigation can pass vacuously.** If the destination first renders short (a body still loading), the browser clamps `scrollY` to 0 on its own, so the test passes with the scroll-to-top removed. Make the destination long *and* already cached (open it in-app once first), then prove the test fails with the fix commented out — comment out the fix's body line, not the hook call: the e2e `webServer` runs `next build`, whose type-check fails on the now-unused function.
