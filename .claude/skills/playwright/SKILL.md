...
**Produce a regression guard's precondition in the test, not from a neighbouring component.** A FLIP-jank spec sourced its "something above the list shifted" precondition from whichever card happened to sit above the list, and broke the day that card moved to another view — the guard is about the list's own measurement, so inject the shift directly (`page.evaluate` inserting a spacer) and the test survives its neighbours.
