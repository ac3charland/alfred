...
- **A sandbox whose pre-installed Chromium predates the pinned one: bridge the paths, don't re-download.** In a web/cloud sandbox `PLAYWRIGHT_BROWSERS_PATH=/opt/pw-browsers` may hold an *older* build (say `chromium-1194`) than the pinned `@playwright/test` resolves (`chromium-1223`), so `setup-chromium.mjs` sees no binary, runs `npx playwright install chromium`, and dies with no CDN access. Symlink the expected version dirs at the installed one — both `chromium-<pinned>/chrome-linux64` **and** `chromium_headless_shell-<pinned>/chrome-headless-shell-linux64`, plus a `chrome-headless-shell` alias for the older layout's `headless_shell` binary — then `touch INSTALLATION_COMPLETE` in each bridged version dir. That last step is what makes `setup-chromium.mjs` report "already installed" and skip the download, so the repo's own `npm run test:e2e` — and therefore the **pre-push hook** — passes instead of needing a direct `npx playwright test`. Storybook snapshots are unaffected: they render inside the pinned Playwright container, which carries its own browser.
