...
- **`renderWithProviders` mounts every module's store provider, including `ReaderProvider` (whose `refresh()` / `reconcileHealth()` fire on `focus` and `visibilitychange`) and `CommsProvider` (which re-reads its whole view on `visibilitychange`, a bfcache-restored `pageshow`, `online`, its channels joining, and a retry timer while it is not live).** A suite that both `jest.mock('@/lib/api-client')` (automock → `undefined`) and fires one of those must stub EVERY fetch they call — `fetchReaderPosts`, `fetchReaderHealth`, and `fetchCommsSnapshot` resolving a `CommsSeed` (`makeCommsSeed(...)`) — or it dies with `TypeError: Cannot read properties of undefined` pointing at a store that isn't even under test (`components/comms/comms-queue-view.test.tsx`). Adding a fetch to any store's return handler therefore breaks other modules' suites: stub the new wrapper there rather than touching the store.
