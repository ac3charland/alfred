...
- **`renderWithProviders` mounts every module's store provider, including `ReaderProvider` and `CommsProvider`, whose `refresh()` / `reconcileHealth()` fire on `focus` and `visibilitychange`.** A suite that both `jest.mock('@/lib/api-client')` (automock → `undefined`) and dispatches one of those events must stub EVERY fetch those handlers call — `fetchReaderPosts`, `fetchReaderHealth`, `fetchCommsHealth` — or it dies with `TypeError: Cannot read properties of undefined (reading 'then')` pointing at a store that isn't even under test (`components/comms/comms-queue-view.test.tsx`). Adding a fetch to any store's focus handler therefore breaks other modules' suites: stub the new wrapper there rather than touching the store.
