...
- **`renderWithProviders` mounts every store provider, including `ReaderProvider` and `CommsProvider`, whose `refresh()` / `reconcileHealth()` fire on `focus` and `visibilitychange`.** A suite that both `jest.mock('@/lib/api-client')` (automock → `undefined`) and dispatches one of those events must also stub the fetch those stores call (`jest.mocked(api).fetchReaderPosts.mockResolvedValue([])`, `fetchCommsHealth` likewise), or it dies with `TypeError: Cannot read properties of undefined (reading 'then')` from a store that isn't even under test (`components/comms/comms-queue-view.test.tsx`).
