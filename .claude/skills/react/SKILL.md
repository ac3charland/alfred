...
- **`useSyncExternalStore`.** The right API for subscribing to an *external* store (Redux, Zustand). Alfred's client cache is a React **Context** store (`useReducer` + Context, see the **`data-flow`** skill), not an external store, so this hook isn't needed for it. It IS used in `inbox-screen.tsx` to subscribe to `matchMedia` (a genuine external source), and for **clock-derived state that has to survive the browser tab sleeping** (`useNow`, `useCommsLive`): `subscribe` arms a plain `setInterval` that re-checks roughly every second — never a single `setTimeout` timed for one future deadline, which is monotonic and can sit pending for hours if the machine actually suspends — and `getSnapshot` returns a value *derived* from `Date.now()` that's stable between calls — a boolean for whether the view is still live (`useCommsLive`), or a time snapped to a tick boundary (`useNow`) — never a raw `Date.now()`, which changes on every call and loops the store. Because `getSnapshot` is what decides whether anything changed, the frequent interval costs a re-check, not a re-render, whenever the derived value hasn't moved. The `useState` + effect alternative trips `react-hooks/set-state-in-effect`, and syncing via a ref during render trips `react-hooks/refs`.
