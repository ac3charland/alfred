...
The trap behind (1): a `subject: a, b, c` colon-list is right only when those are genuine **subject-nouns** of a broad *reference* skill (`the Messages API, tool definitions, streaming`; `database → API → store → components`). A **house-style / conventions skill whose content is a handful of rules has no such inventory to list** — so name the subject in a single phrase and stop. If your list reads as the advice itself ("what to keep vs. leave behind, grounding, handling drift, test coverage"), that's the *body* leaking into the description; delete it. **After drafting, re-read your own description against this and cut anything that states what the skill _says_ rather than what it's _about_** — this is the step that's skipped most, even right after reading this rule.
