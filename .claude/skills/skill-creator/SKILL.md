...
The description is the only thing Claude sees when deciding whether to reach for a skill, so it's worth getting right. The checklist below is the *what to do*; the reasoning, evidence, and caveats behind each rule live in [`references/description-triggering.md`](references/description-triggering.md) — read it when a rule isn't obvious. **Before you finalize a description — when creating *or* editing a skill — skim [`references/description-examples.md`](references/description-examples.md), a before/after library of real fixes, and check your draft against the smell that's easiest to fall into: inlining the skill's content (its rules/steps) instead of naming its subject.** When you fix a description after review, add the before/after to that library so the next author dodges it.

**Quick check before you write it — the three that get missed most** (the rest of the list elaborates, but these are the ones that bite, and position bias means they have to lead): **(1)** name the *subject* the skill covers — never restate its rules or guidance, even nominalized; **(2)** cut redundant scope from the front (the repo name especially — the agent already knows the repo, and skill-lint now errors on it); **(3)** front-load the distinctive keywords into the first ~250 characters before truncation.

The trap behind (1): for a **house-style / conventions skill whose content is a handful of rules**, there's no inventory of subject-nouns to list — a `subject: a, b, c` clause is right only when those are genuine subjects of a broad *reference* skill (`the Messages API, tool definitions, streaming`). So name the subject in a single phrase and stop; if your list reads as the advice itself, that's the *body* leaking into the description. **After drafting, re-read your own description and cut anything that states what the skill _says_ rather than what it's _about_** — the step that's skipped most, even right after reading this rule (the worked example is in `references/description-examples.md`).
...
- **Describe the skill; don't inline its content.** The description says *what the skill covers and when to reach for it* — not the rules or instructions themselves. Naming the subject area is fine ("covers the Messages API, tool definitions, streaming"); enumerating the skill's actual guidance — even nominalized ("the body-vs-references split, avoiding duplication, not narrating the edit") — just reproduces the body, drifts from it, and wastes the triggering budget. State the subject, not the guidance.
...
- `references/description-examples.md` — a before/after library of real description fixes (inlining the skill's content, enumerating every rule, naming the repo), each with the rule it broke. Skim it whenever you write or edit a description, and append the before/after when you fix one after review.
