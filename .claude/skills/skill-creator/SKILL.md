...
**Quick check before you write it — the three that get missed most** (the rest of the list elaborates, but these are the ones that bite, and position bias means they have to lead): **(1)** name the *subject* the skill covers — never restate its rules or guidance, even nominalized; **(2)** cut redundant scope from the front (the repo name especially — the agent already knows the repo, and skill-lint errors on it); **(3)** front-load the distinctive keywords into the first ~250 characters before truncation.
