...
description: >
  ...
  description into classes. For scaffolding or composing shadcn/ui components use the shadcn-ui
  skill; for animation and transition tokens use the motion skill.
---
...
| **Segmented control that must fit a narrow container** | `flex w-full gap-0.5` on the group; `flex-auto min-w-0 truncate text-center` + tight `px-*` on each segment | `flex-auto` (basis: content) starts each segment at its own label's width and shares out only the leftover; `flex-1` (basis: 0) gives equal shares sized to the *narrowest* label and clips the longest. `min-w-0 truncate` keeps a future overflow inside the control instead of pushing it past its container. Size the padding from the real budget — container width minus label text minus gaps — not by eye. |
