...
- **Do not bypass the hooks** — never `git commit --no-verify` or
  ...
  bundled tool (`npm run batch-commit -- <file>`): it runs the full `check:fast`
  gate **once** against the complete working tree, validates every commit message
