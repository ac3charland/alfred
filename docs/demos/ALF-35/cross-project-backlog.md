...
![Reordering the Backlog: each chevron swap glides smoothly via FLIP (no mid-flight jump)](cross-project-backlog-video-2.gif)

The glide above is **jank-free**: an earlier version stuttered because the optimistic swap's server reconcile re-ran the FLIP layout effect mid-transition, snapping the row ~¾ of the way in one frame. The `debug-animations` probe pinned it by sampling the moved row's `top` every frame — flat at the old slot, then a single ~37px jump, then easing. The fix bails the effect out on a same-order re-render (and measures "Last" cleanly); the same probe now shows a smooth ~5px-per-frame ease, and `e2e/code-backlog-reorder-flip.spec.ts` asserts no single frame covers more than 40% of the journey.
