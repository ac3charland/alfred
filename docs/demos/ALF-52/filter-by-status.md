...
The Backlog header's old **Show completed** toggle is replaced by a **Filter by status** dropdown: a multi-select checkbox menu with one entry per factory state (the six happy-path lanes plus the off-board Blocked/Abandoned). The selection drives `useBacklog({ statuses })`, which lists only the stories whose status is checked. It defaults to the outstanding states, so `done`/`abandoned` stay hidden until the owner checks them — preserving the prior default while making every other status filterable. The default selection is the resting state, so the trigger stays neutral with no count; any other selection (narrower or wider) turns the trigger teal and shows the count of selected statuses.

![](filter-by-status-image-1.png)

Default view: the trigger reads **Filter by status** with no count (the resting state) and the menu shows the six outstanding states checked, with **Done** and **Abandoned** unchecked.
