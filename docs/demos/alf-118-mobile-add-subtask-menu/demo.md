...
- An **"Add subtask"** item (with a `+` glyph) now sits in the `⋯` menu on mobile, **below the divider under "Open details"**. It's `md:hidden` so desktop — where the visible `+` remains — never doubles up, and it's rendered for **task rows only** (code/unclassified rows nest no subtasks).
...
Opening a task row's `⋯` menu on mobile — **"+ Add subtask" sits below the divider, under "Open details"**:
