 ...
 * does, inserts three posts either side of the retention window, and calls `reader_sweep_text`
 * the way the Worker does — one batch per call, until a batch reports 0.
 *
...
/** Three posts: two past the ninety-day window, one comfortably inside it. */
const POSTS = [
      ...
      await client.query(
        `insert into reader_posts
         ...
         values ($1, $2, 'gmail-personal', $4, 'A post worth keeping the summary of',
                 ...
                 repeat('the stored body of the post. ', 40), 240, true,
        ...
        [post.id, PUBLICATION_ID, post.days, post.id],
      );
    }
