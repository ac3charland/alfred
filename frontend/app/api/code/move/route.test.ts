...
const MOVED_ROW = { item_id: 'i1', ref: 'ALF-1', priority: -1 };
...
/** Supabase stub: `move_code_priority` is a setof RPC, awaited directly (no `.single()`). */
...
function postRequest(body: unknown): Request {
  return new Request('http://localhost/api/code/move', {
    method: 'POST',
...
describe('POST /api/code/move', () => {
  ...
  it('calls move_code_priority with the ref and direction and returns the updated row', async () => {
    ...
    expect(mockSupabase.rpc).toHaveBeenCalledWith('move_code_priority', {
      p_ref: 'ALF-1',
      p_to_top: true,
    });
  ...
  it('forwards to_top false for a jump to the bottom', async () => {
    const mockSupabase = makeMockSupabase(TEST_USER, { data: [MOVED_ROW], error: undefined });
    mockCreateClient.mockResolvedValue(mockSupabase as never);
  ...
  it('maps a Supabase error to its status', async () => {
    const mockSupabase = makeMockSupabase(TEST_USER, {
      data: undefined,
      error: { message: 'move_code_priority: unknown ref' },
    });
    mockCreateClient.mockResolvedValue(mockSupabase as never);
