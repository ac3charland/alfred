 ...
 * `ProjectNav` for Code, `CommsNav` for Comms, `ReaderNav` for Reader, `WikiNav` for Wiki,
 * `FolderNav` for Tasks —
 * each reading from its already-seeded store. Switching modules re-derives this with no remount
 * or refetch.
 */
export function ShellNav() {
  ...
  if (current === 'reader') return <ReaderNav />;
  if (current === 'wiki') return <WikiNav />;
  return <FolderNav />;
}
