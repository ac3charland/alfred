...
/**
 * One row of a **triage list** — the cross-cutting views (By Priority, Today) that gather tasks
 * from every folder into a single ranked list. A normal, actionable task component rather than a
 * read-only index entry: it carries a completion checkbox (opening the cascade modal when the
 * task hides active subtasks) and — when it has subtasks — an expand chevron that reveals them,
 * recursively, each rendered by this same component. The right-hand task-row affordances
 * (add-subtask, the ⋯ menu) are deliberately omitted: a triage list is for working the list down
 * and ticking things off, not for restructuring.
 *
 * The title stays a real `<a href>` that jumps to the task in its containing view and rings it
 * there via {@link navigateToTaskAndFocus}, so ⌘/middle-clicks still open a new tab. The priority
 * chip (level, or a muted "Set priority" prompt) opens its own picker to re-prioritise in place.
 * Only a depth-0 row shows the folder/Inbox label — subtasks share their root's bucket.
 *
 * Must be mounted under a `TasksProvider` (the shell-seeded store).
 */
