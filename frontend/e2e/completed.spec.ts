...
test('shows completed tasks with their context labels', async ({ page, seed }) => {
  await seed({
    ...
    items: [
      makeItem('Finished inbox task', { id: 'd1', status: 'completed' }),
      makeItem('Finished work task', { id: 'd2', status: 'completed', folder_id: 'f1' }),
    ],
  });
...
test('reactivating a completed task returns it to the inbox', async ({ page, seed }) => {
  await seed({ items: [makeItem('Reopen me', { id: 'd1', status: 'completed' })] });
