...
/**
 * Code filter store — the single source of truth for each Code view's filter selections: which
 * statuses are shown, and (ALF-156) which projects the Backlog lists.
 *
 * A view's filter is a cross-navigation invariant: the Backlog and the project boards
...
export interface CodeFilterState {
  ...
  /** Selected project ids per view key, same absent-means-default rule (ALF-156). */
  projectsByKey: ReadonlyMap<string, readonly string[]>;
}

interface CodeFilterActions {
  /**
   * Set the status selection for `key`. `defaults` seeds the base a functional `update` builds on
   * the first time a view is touched (before any entry exists for its key), so a toggle from the
  ...
  /** Set the project selection for `key`, with the same `defaults` seeding rule (ALF-156). */
  setProjectIds: (
    key: string,
    defaults: readonly string[],
    update: React.SetStateAction<readonly string[]>,
  ) => void;
}

/**
 * Store `update` under `key`, resolving a functional update against the entry already there — or
 * against `defaults` on the first touch of that key.
 */
function withSelection<TValue>(
  current: ReadonlyMap<string, readonly TValue[]>,
  key: string,
  defaults: readonly TValue[],
  update: React.SetStateAction<readonly TValue[]>,
...
export function CodeFilterProvider({ children }: { children: React.ReactNode }) {
  ...
  const [projectsByKey, setProjectsByKey] = React.useState<ReadonlyMap<string, readonly string[]>>(
    () => new Map(),
  );

  const state = React.useMemo<CodeFilterState>(
    () => ({ byKey, projectsByKey }),
    [byKey, projectsByKey],
  );
  ...
  const actions = React.useMemo<CodeFilterActions>(
    () => ({
      setStatuses(key, defaults, update) {
        setByKey((current) => withSelection(current, key, defaults, update));
      },
      setProjectIds(key, defaults, update) {
        setProjectsByKey((current) => withSelection(current, key, defaults, update));
      },
    }),
    [],
  );
...
/** Read every view's stored filter selections. Throws outside a provider. */
export function useCodeFilters(): CodeFilterState {
  return useStateValue('useCodeFilters');
}

/** Read the set-selection actions. Throws outside a provider. */
