...
/**
 * Alternation order matters: each `epic-` phase ENDS with the story phase of the same name, and
 * JS alternation is first-match-wins, so listing `refinement` (or `implementation`) first would
 * match the tail of `phase: epic-refinement` (the `[ \t]*` separator doesn't anchor the value's
 * start) and route an epic PR at a story. Keep both longer phases ahead of their suffixes.
 * `spike` shares no suffix with the others, so its position in the list is free.
 */
const PHASE_RE =
